...
if(@$_SESSION['id_user'] && @$_SESSION['id_rol'] == 1) {
    ...
    function set_user($name, $user, $password, $id_rol, $state) {
        global $pdo;
        $sql = "INSERT INTO users (name, user, password, id_rol, state) VALUES (:name, :user, :password, :id_rol, :state)";
    ...
    function get_applicants_super(){
        global $pdo;
        $sql = "SELECT workplaces.work_position, applicants.* from workplaces INNER JOIN applicants ON workplaces.id = applicants.id_workplace AND applicants.id_denomination = (SELECT MAX(id) from announcements WHERE state = 1) AND applicants.accepted = 0 ORDER BY applicants.date_time";
        $query = $pdo->prepare($sql);
        if($query->execute()) {
            return $query->fetchAll();
        }
    ...
    function accept_applicant($id) {
        global $pdo;
        $sql = "UPDATE applicants SET accepted = 1 WHERE id = :id";
        $update = $pdo->prepare($sql);
        $update->bindParam(":id", $id);
        if($update->execute()) {
            return [
    ...
    function decline_applicant($id) {
        global $pdo;
        $sql = "UPDATE applicants SET accepted = 2 WHERE id = :id";
        $update = $pdo->prepare($sql);
        $update->bindParam(":id", $id);
        if($update->execute()) {
            return [
    ...
    function set_applicant($nombres, $apellidos, $dni, $id_plaza, $telefono, $documento) {
        ...
        if($result_verify != null) {
            if($result_verify[0] > 3) {
                echo "Ya se a llegado al maximo de postulaciones por persona";
                return [
                    "code" => 3,
                    "msg" => "Maximo de intentos por dni"
                ];
            }else {
                $size_documento = $documento['size'];
                ...
                $destino = PATH_FILES . "//applicants//" . $hoy_hora . "." . explode(".", $documento['name'])[1];
                $nombre_archivo = $hoy_hora . "." . explode(".", $documento['name'])[1];
                    if(move_uploaded_file($ruta_temporal, $destino)) {
                        ...
                        $sql = "INSERT INTO applicants(
                            ...
                            :documento,
                            NOW(),
                            (SELECT MAX(id) from announcements WHERE state = 1), 0)";
                        $insert = $pdo->prepare($sql);
